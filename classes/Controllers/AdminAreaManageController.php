 ...
 * ECJIA 地区列表管理文件
 */
class AdminAreaManageController extends AdminBase
{
    private $db;
    ...
    public function __construct()
    {
        ...
        $this->db = RC_Model::model('shipping/region_model');
        ...
        RC_Script::enqueue_script('admin_region', RC_App::apps_url('statics/js/admin_region.js', $this->__FILE__), array(), false, true);
        RC_Script::localize_script('admin_region', 'js_lang_admin_region', 'app-setting::jslang.admin_region');
    }
    ...
    public function init()
    {
        ...
        ecjia_screen::get_current_screen()->set_help_sidebar(
            ...
            '<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:系统设置#.E5.9C.B0.E5.8C.BA.E5.88.97.E8.A1.A8" target="_blank">关于地区设置帮助文档</a>') . '</p>', 'setting');
        ...
        if (!empty($id)) {
            ...
            $this->assign('action_link', array('href' => RC_Uri::url('setting/admin_area_manage/init', 'id=' . $parent_id), 'text' => __('返回上级', 'setting')));
        }

        return $this->display('area_list.dwt');
    }
    ...
    public function add_area()
    {
        ...
        if ($is_only) {
            return $this->showmessage(__('抱歉，已经有相同的地区名存在！', 'setting'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        } else {
            $data = array(
                'parent_id'   => $parent_id,
            ...
            if ($region_id) {
                ...
                return $this->showmessage(__('添加新地区成功！', 'setting'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('setting/admin_area_manage/init', 'id=' . $parent_id)));
            } else {
    ...
    public function edit_area_name()
    {
        ...
        if ($is_only) {
            ...
            if ($this->db->where(array('region_id' => $id))->update(array('region_name' => $region_name))) {
                ...
                return $this->showmessage(__('修改名称成功！', 'setting'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('setting/admin_area_manage/init', 'id=' . $parent_id)));
            } else {
                return $this->showmessage($this->db->error(), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        }
    }
    ...
    {
        $db = RC_Model::model('shipping/region_model');
