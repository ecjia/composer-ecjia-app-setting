		...
		foreach ($_FILES AS $code => $file) {
			...
			if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {				
				if ($file_var_list[$code]['value']) {
				    AdminSettingAction::replaceFile($code, $file_var_list[$code]['value']);
				}
...
//				$save_name = $code == 'icp_file' ? substr($file['name'],0, strrpos($file['name'], '.')) : $code;
				...
				if ($code == 'icp_file') {
                    $filetype = 'file';
                    $save_name = substr($file['name'],0, strrpos($file['name'], '.'));
                } else {
                    $filetype = 'image';
                    $save_name = $code;
                }
...
//				$filetype = $code == 'icp_file' ? (strrpos(RC_Config::load_config('filesystems', 'UPLOAD_FILE_EXT'), $extname) ? 'file' : 'image') :'image';
				...
				$upload = RC_Upload::uploader($filetype, array(
				    'save_path' => 'data/assets/'.ecjia::config('template'),
                    'replace'=> AdminSettingAction::isReplaceFile($code),
                    'auto_sub_dirs' => false)
                );
				$upload->add_filename_callback(function () use ($save_name){
				    return $save_name;
				});
	...
	public function del()
    {
		...
		$model = ecjia_config::getRepository()->getTableModel();
		$img_name     = $model->where(array('code' => $code))->value('value');
