		...
		<div class="list-div list media_captcha wookmark warehouse" id="listDiv">
			...
			<!-- {foreach from=$region_arr item=region} -->
				<li class="thumbnail">
					...
					<div class="input">
						<a class="data-pjax no-underline" title='{t domain="setting"}进入{/t}' href="{url path='setting/admin_area_manage/init' args="id={$region.region_id}"}"><i class="fontello-icon-login"></i></a>
						<a class="no-underline" title='{t domain="setting"}编辑{/t}' value="{$region.region_id}" data-toggle="modal" href="#editArea" data-name="{$region.region_name}"><i class="fontello-icon-edit"></i></a>
						<a class="ajaxremove no-underline" data-toggle="ajaxremove" data-msg='{t domain="setting" 1={$region.region_name}}您确定要删除此地区[%1]吗？删除此地区将会同时删除此地区下的所有地区！{/t}' href='{url path="setting/admin_area_manage/drop_area" args="id={$region.region_id}"}' title='{t domain="setting"}删除一级地区{/t}'><i class="fontello-icon-trash ecjiafc-red"></i></a>
					</div>
				</li>
				<!-- {/foreach} -->
				...
				<form class="form-horizontal" method="post" name="editArea" action="{url path='setting/admin_area_manage/edit_area_name'}">
					<fieldset>
						<div class="control-group formSep">
							<label class="control-label old_warehouse_name" for="user_name">{t domain="setting"}原地区名：{/t}</label>
							<div class="controls">
								<span class="parent_name"></span>
							</div>
						</div>
						<div class="control-group formSep">
							<label class="control-label new_warehouse_name" for="user_name">{t domain="setting"}新地区名：{/t}</label>
							<div class="controls">
		...
		<div class="modal-body h250">
			<div class="row-fluid">
				...
				<form class="form-horizontal" method="post" name="addArea" action="{url path='setting/admin_area_manage/add_area'}">
					<fieldset>
						<div class="control-group formSep">
							<label class="control-label" for="user_name">{t domain="setting"}新增地区名：{/t}</label>
							<div class="controls">
								<input type="text" name="region_name" value=""><br>
								<span class="help-block">{t domain="setting"}地区名称不能重复{/t}</span>
							</div>
						</div>
						<div class="control-group t_c">
							<button class="btn btn-gebo" type="submit">{t domain="setting"}确定{/t}</button>
